    ...
    def _get_exception_name(self, raises):
        ...
        if isinstance(raises.exc, ast.Name):
            ...
            if hasattr(raises.exc.func, "id"):
                ...
                logger.debug("Raises function call has neither id nor attr." "has only: %s" % str(dir(raises.exc.func)))
    ...
    def add_exception(self, node):
        ...
        if name == "":
            if self.bare_handler_exceptions is not None:
                ...
                logger.warning("Unexpectedly had no exception name raised and no exception " "in context.")
...
class RaiseVisitor(ast.NodeVisitor):
    ...
    def visit_Try(self, node):
        ...
        for handler in node.handlers:
            if handler.type:
                if handler.name and (isinstance(handler.type, ast.Name) or isinstance(handler.type, ast.Tuple)):
